    ...
    public partial class FingerprintService : IDisposable
    {
        ...
        public async Task<bool> StartCapture()
        {
            try
            {
                ...
                try
                {
                    // Verificar se já está capturando
                    _capturer.StartCapture();
                    OnStatusChanged?.Invoke(this, "⏳ Posicione o dedo no leitor (máximo 10 segundos)...");
                    
                    // Aguardar captura com timeout reduzido
                    int timeout = 0;
                    while (_isCapturing && timeout < 100)
                    {
                    ...
                    if (timeout >= 100)
                    {
                        OnStatusChanged?.Invoke(this, "⏰ Tempo limite excedido");
                    }
